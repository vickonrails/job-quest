...
export function TableHeader<T>({ columns, hideActions }: { columns: Column<T>, hideActions?: boolean }) {
    return (
        <thead className="border-b-2 rounded-sm bg-white">
            <tr>
                {!hideActions && (
                    <th className="border border-b-2 p-4 text-left rounded-tl-lg bg-white">
                        <Typography variant="body-sm" className="text-light-text font-normal">Actions</Typography>
                    </th>
                )}
                {columns.map(col => (
                    <th className="border border-b-2 p-4 text-left" key={col.header}>
